...
<head>
    <meta charset="utf-8">
    <title>Бронирование — {{ $hall->name }}</title>
</head>
<body style="font-family: Arial, sans-serif; margin: 40px;">
    <h1>Бронирование: {{ $hall->name }}</h1>
    <p><a href="{{ route('halls.show', $hall) }}">← Назад</a></p>
    ...
    <form method="POST" action="{{ route('bookings.store') }}" id="bookingForm">
        ...
        @foreach($equipment as $eq)
            <div style="margin-bottom: 10px;">
                <b>{{ $eq->name }}</b>
                (всего: {{ $eq->total_qty }}, {{ $eq->price_per_hour }} ₽/час за 1 шт)
                <br>
                <input type="number" min="0" max="50" name="equipment[{{ $eq->id }}]" value="{{ old('equipment.' . $eq->id, 0) }}">
            </div>
        @endforeach
...
<script>
(function() {
    const apiAvailabilityUrl = "{{ route('api.halls.availability', $hall) }}";
    const apiEquipmentUrl = "{{ route('api.halls.equipment_availability', $hall) }}";
